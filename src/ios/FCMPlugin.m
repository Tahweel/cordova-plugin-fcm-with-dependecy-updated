...
static NSString *setSessionCallback = @"FCMPlugin.setSession";
...
-(void) setSession:(CDVInvokedUrlCommand *)command
{
    ...
    if(params != nil) {
        if(params.count >= 3) {
            ...
            NSString *group = [mainBundle objectForInfoDictionaryKey:@"group"];
            
            NSUserDefaults *sharedDefaults = [[NSUserDefaults alloc] initWithSuiteName:group];
            
            [sharedDefaults setObject:params[0] forKey:@"token"];
            [sharedDefaults setObject:params[1] forKey:@"deviceId"];
            [sharedDefaults setObject:params[2] forKey:@"baseURL"];
